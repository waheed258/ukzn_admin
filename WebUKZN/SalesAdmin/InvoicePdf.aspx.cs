...
public partial class SalesAdmin_InvoicePdf : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (objDs.Tables.Count > 0)
                {
                    ...
                    {
                        string strimgLogo = _objBOUtiltiy.LogoUrl("Serendipitylogo.png");
                        string strIataLog = _objBOUtiltiy.LogoUrl("IATA_Logo.svg.png");
                        string strASTA = _objBOUtiltiy.LogoUrl("ASTA.png");
                        readFile = readFile.Replace("{CompanyName}", "Serendipity travel");
                        readFile = readFile.Replace("{address}", "2nd Floor, Silver Oaks Bldg 36 Silverton Road,Musgrave<br/>Email : media@serendipitytours.co.za <br/>Telephone Number	: +27 31 201 0630<br/> Fax : (031) 201 0646");
                        readFile = readFile.Replace("{Image}", "<img   src='" + strimgLogo + "'></img>");
                        readFile = readFile.Replace("{Image3}", " ");
                        readFile = readFile.Replace("{IATA}", "<img   src='" + strIataLog + "'></img>");
                        readFile = readFile.Replace("{ASTA}", "<img   src='" + strASTA + "'></img>");
                        ...
                        //readFile = readFile.Replace("{OrderNo}", objDs.Tables[5].Rows[0]["OrderNo"].ToString());
                    }
                    else
                    {
        ...
        try
        {
            ...
            if (objDs.Tables[0].Rows.Count > 0)
            {
                ...
                if (objDsQut.Tables[2].Rows.Count > 0)
                {
                    ...
                    objTravelRequest.invoicedescription = "Invocie order no " + objDsQut.Tables[2].Rows[0]["ukzn_orderno"].ToString();
                ...
                if (objDsQut.Tables[0].Rows.Count > 0)// Flight Invoice
                {
                    foreach (DataRow QuotationRow in objDsQut.Tables[0].Rows)
                    {
                        ...
                        objF.invoiceitemnumber = "F_" + QuotationRow["FlightRequestId"].ToString();
                ...
                if (objDsQut.Tables[1].Rows.Count > 0)// Hotel Invoice
                {
                    foreach (DataRow QuotationRow in objDsQut.Tables[1].Rows)
                    {
                        ...
                        objH.invoiceitemnumber = "A_" + QuotationRow["HotelRequestId"].ToString();
